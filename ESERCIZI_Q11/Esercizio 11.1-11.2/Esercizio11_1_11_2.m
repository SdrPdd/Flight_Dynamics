% Condizione di volo equilibrato a quota costante, a comandi bloccati, per
% assegnate quote e velocità determinando la deflessione corrispondente
...
if (myAC.err == -1)
    ...
    Aeq(3,3) = 1;
    beq = zeros(4,1);
    delta_s0 = convang(-1.500,'deg','rad');
    beq(3,1) = delta_s0;
    ...
    disp(['Velocità angolare di beccheggio ' num2str(convangvel(q0,'rad/s','deg/s')) ' deg/s'])
    disp(['Angolo di deflessione dell''equilibratore ' num2str(delta_e0_deg) ' deg'])
...
t_fin = 30;         %Durata dell'intera fase di volo [s]
t_sim_first = 1;    %Durata della prima fase di volo [s]
...
% Supposto per 0 < t < t_sim_first = 1
% Per cui la variazione di delta_e risulta essere nulla
...
%Il valore nullo di delta_e ricavato dalla consizione precedente
%costituisce una delle condizioni iniziali per la successiva fase di volo.
...
delta_e_float = mState_fin(end,8); %LO LASCIO?

...
% Considerare le seguenti leggi temporali:
% velocità angolare di rollio p(t) da 0 a p_max e di nuovo a 0
% velocità angolare di beccheggio q(t) analoga con q_max=0.5*p_max
% velocità angolare di imbardata r(t) nulla
...
p_cost=0.5; %valori in rad/s
q_cost=1;
r_cost=0;
p=@(t) interp1([0, t_f/30, t_f/10, t_f/5, 0.7*t_f, 0.9*t_f, ...
       t_f],[p_cost, p_cost, p_cost, p_cost, p_cost, p_cost, p_cost],...
       t,'pchip'); %Velovictá angolare di rollio in rad/s
q=@(t) interp1([0, t_f/30, t_f/10, t_f/5, 0.8*t_f, 0.9*t_f, ...
       t_f],[q_cost, q_cost, q_cost, q_cost, q_cost, q_cost, q_cost],...
       t,'pchip'); %Velovictá angolare di rollio in rad/s
r=@(t) 0*(t); %Velocitá angolare di imbardata in rad/s
